    ...
    <div class="user-header">
      ...
      <div class="button-right">
        <el-dropdown trigger="click">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item @click="echartsVisible = 1"
                >显示数据为表格</el-dropdown-item
              >
              ...
              >
              <el-dropdown-item @click="echartsVisible = 5" divided
                >全部显示</el-dropdown-item
              >
            </el-dropdown-menu>
          </template>
        </el-dropdown>
    ...
    <div v-if="echartsVisible == 1 || echartsVisible == 5">
      <div>
        ...
        >
          ...
          />
          <el-table-column
            prop="productSkusTitle"
            label="耗材型号"
            width="180"
          ></el-table-column>
          <el-table-column
            prop="sumProductNumber"
            label="领用数量"
            width="180"
            sortable
          />
          <el-table-column
            prop="countOrderNumber"
            label="订单数量"
            width="180"
            sortable
          />
...
import {
  listDateOrderProductCountNoGroupAPI,
  listDateOrderProductCountNoGroupAllListAPI,
...
const getList = () => {
  if (defaultList.value.dateState == 2) {
    listDateOrderProductCountNoGroupAPI(defaultList.value)
      .then((res) => {
    ...
    listDateOrderProductCountNoGroupAPI(defaultList.value)
      .then((res) => {
        ...
        if (tableData.value != null) {
          for (let i = 0; i < tableData.value.length; i++) {
            tableData.value[i].timeFrame = "全部时间";
          }
        }
      })
      .catch(console.log("false"));
  ...
  store.commit(
    "SET_CONSUMABLE_SPECIFIED_TIME1",
    defaultList.value.specifiedTime1
  );
  store.commit(
    "SET_CONSUMABLE_SPECIFIED_TIME2",
    defaultList.value.specifiedTime2
  );
...
const getAllList = () => {
  ...
  if (defaultList.value.dateState == 2) {
    listDateOrderProductCountNoGroupAllListAPI(defaultList.value).then(
      (res) => {
...
const querySearch = (queryString, cb) => {
  ...
  listDateOrderProductCountNoGroupAPI(querySearchList.value).then((res) => {
