...
const getList = () => {
    if (defaultList.value.dateState == 2) {
        listDateOrderProductCountNoGroupAPI(defaultList.value).then(res => {
...
const getAllList = () => {
    ...
    if (defaultList.value.dateState == 2) {
        listDateOrderProductCountNoGroupAllListAPI(defaultList.value).then(res => {
...
const handleDowloadPage = () => {
    import('@/utils/Export2Excel').then(excel => {
        const tHeader = ['耗材类型', '耗材型号', '领用数量', '订单数量', '时间范围']
        const filterVal = ['productTitle', 'productSkusTitle', 'sumProductNumber', 'countOrderNumber', 'timeFrame']
